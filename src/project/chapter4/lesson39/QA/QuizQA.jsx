...
const QuizQA = (props) => {
    ...
    const [cauHoiObj, setCauHoiObj] = useImmer({
        [cauhoiId]: {
            id: uuidv4(), description: '', imageFile: '', imageName: '',
            answers: [dapAnId]
        }
    });
    ...
    const handleAddRemoveAnswer = (type, questionId, anwserId) => {
        ...
        if (type === 'REMOVE') {
            if (cauHoiObj[questionId]) {
                setCauHoiObj(draft => {
                    let newAs = cauHoiObj[questionId].answers.filter(item => item !== anwserId);
                    draft[questionId].answers = newAs;
                })
            }
            if (dapAnObj[anwserId]) {
                setDapAnObj(draft => {
    ...
    const handleOnChangeFileQuestion = (questionId, event) => {
        if (cauHoiObj[questionId] && event.target && event.target.files && event.target.files[0]) {
            setCauHoiObj(draft => {
    ...
    const handleSubmitQuestionForQuiz = async () => {
        // console.log(">>>> check state Q: ", cauHoiObj);
        // console.log(">>>> check state A: ", dapAnObj);
    ...
    console.log(">>> check cauHoiObj: ", cauHoiObj)
    console.log(">>> check dapAnObj: ", dapAnObj)
                                        ...
                                        return (
                                            <div key={keyA} className='answers-content'>
                                                <input
                                                    ...
                                                    onChange={(event) =>
                                                        handleAnswerQuestion('CHECKBOX', keyA, cauHoiObj[keyQ].id, event.target.checked)}
                                                />
                                                <div className="form-floating anwser-name">
                                                    <input
                                                        ...
                                                        placeholder="name@example.com"
                                                        onChange={(event) =>
                                                            handleAnswerQuestion('INPUT', keyA, cauHoiObj[keyQ].id, event.target.value)}
                                                    />
                                                ...
                                                <div className='btn-group'>
                                                    ...
                                                    {cauHoiObj[keyQ].answers.length > 1 &&
                                                        <span onClick={() => handleAddRemoveAnswer('REMOVE', cauHoiObj[keyQ].id, keyA)}>
